...
def process_instruction(instr, line_num, state, logger):
    ...
    if state.ANNOTATION_FLAG:
        ...
        # Handle the block
        if state.block_term:
            logger.info("Previous instruction called for block termination -- terminating block and creating a new one")
            ...
            if state.active_block != None:
                ...
                logger.info("Pushed block {} to method {}".format(state.active_block.block_id, state.active_method.method_name))
        ...
        if op_map["method_start"].match(instr):
            # On matched method_start we want to end the old method and start a new one (basic block should already be pushed to method by above code)
            logger.info("Method matched - {}".format(instr))
            if state.active_method != None: # The first method of a file will have no previous method to push, so we need to check for this case    
                ...
                logger.info("Pushed previous method {} to class".format(state.active_method.method_name))
            ...
            # label resolution
            logger.info("Attempting to resolve labels for {}".format(state.active_method.method_name))
            ...
            logger.info("Encountered invocation!")
            state.block_term = True
            ...
            logger.info("Instruction fallthrough: {}".format(instr))
            instruction = Instruction(
                    instr, 
                    "other", 
                    state.instruction_id, 
